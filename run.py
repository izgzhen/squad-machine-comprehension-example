...
class Run(object):
    ...
    def step(self, supervisor, sess, batch_size=None, save_summary=False, val=False):
        ...
        # zhen: evaluation site
        # input is the prediction result, an emb_mat and some {start, end} indices thing?
        e = SquadEvaluation(data, inputs=input_vals, outputs=output_vals, loss=loss_val, global_step=global_step_val)
        return e

    # zhen: what does 'lap' mean?
...
def train(config, val_config=None):
    '''zhen: cli.py: run.train(config, val_config=val_config)
    '''
    # zhen: device is cpu?
    with tf.device("/cpu:0"):
        ...
        # zhen: Supervisor - checkpoints model and computes summaries
        supervisor = tf.train.Supervisor(logdir=config.out_dir,
                                         save_model_secs=config.save_model_secs, summary_op=None)
        # zhen: Coordinator - coordinate the termination of threads
